...
import com.upgenicsint.phonecheck.test.chip.AutoBackCameraTest
import com.upgenicsint.phonecheck.test.chip.AutoFrontCameraTest
...
class AutoCameraTestActivity : DeviceTestableActivity<Test>() {
    ...
    private val isTimeCaptureImage = Loader.instance.isTimeCapture
        ...
        if (intent.hasExtra(CAMERA_TYPE_KEY)) {
            ...
            when (type) {
                0 -> {
                    ...
                    try {
                        test = Loader.instance.getByClassType(AutoBackCameraTest::class.java)
                    } catch (e: Exception) {
                        e.printStackTrace()
                    }
                }
                1 -> {
                    ...
                    try {
                        test = Loader.instance.getByClassType(AutoFrontCameraTest::class.java)
                    } catch (e: Exception) {
                        e.printStackTrace()
                    }
                }
    ...
    private fun reloadFragmentFor(face: CameraFace) {
        ...
        cameraFragment.onCameraOpen = { camera ->
            ...
            if (cameraFragment.currentFace === CameraFace.FACE_FRONT) {
                test.resultsFilterMap.put(Test.frontCameraFlashTestKey, AutoFrontCameraTest.hasFrontFlash)
            }

            /*Take Picture*/
            cameraTakeBtn.setOnClickListener {
            ...
            if (currentFace == CameraFace.FACE_FRONT) {
                ...
                if (isTimeCaptureImage) {
                    Thread(Runnable {
                        ...
                        for (timer in 3 downTo 1) {
                            runOnUiThread {
                                timerText.visibility = View.VISIBLE
                                cameraTakeBtn.isEnabled = false
        ...
        try {
            supportFragmentManager.beginTransaction().add(R.id.fragmentContainer, cameraFragment).commit()
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    private fun onPictureTaken(data: Any) {
        ...
        runOnUiThread {
            ...
            /*if (isAutoFrontEnabled || isAutoRearEnabled) {
    ...
    private fun retakeProcess() {
        cameraPictureImageView.visibility = View.GONE
        hideCameraTakeButton(true)
        hideCameraQualityButtons(true)
        cameraFragment?.onCloseCamera()
        cameraFragment?.openCamera()
    }

    class ImageLoader(activity: AutoCameraTestActivity) : AsyncTask<Any, Bitmap?, Bitmap?>() {
        val activity = WeakReference<AutoCameraTestActivity>(activity)
...
//                }
            ...
            if (bitmap != null) {
                Log.d(TAG, "Before Width " + bitmap.width + " Height " + bitmap.height)
                val scaledBitmap = Bitmap.createScaledBitmap(bitmap, width, height, true)

                Log.d(TAG, "after Width $width Height $height")
                return scaledBitmap
    ...
    override fun closeTimerTest() {
        ...
        try {
            if (Loader.RECORD_TIMER_TASK != null) {
                ...
                if (currentFace == CameraFace.FACE_FRONT) {
                    CameraTestActivity.FRONT_CAMERA_SCREEN_TIME = Loader.TIME_VALUE
                    try {
                    ...
                    CameraTestActivity.REAR_CAMERA_SCREEN_TIME = Loader.TIME_VALUE
                    try {
                        val recordPrefs = getSharedPreferences(resources.getString(R.string.record_tests), Context.MODE_PRIVATE)
    ...
    private fun hideCameraTakeButton(hide: Boolean) {
        cameraTakeBtn.visibility = if (hide) View.GONE else View.VISIBLE
    }
