...
import com.upgenicsint.phonecheck.activities.AutoCameraTestActivity
...
class AutoBackCameraTest(context: Context) : Test(context) {
    ...
    override val title: String
        get() = context.getString(R.string.back_camera)
    ...
    override val activityRequestCode: Int
        get() = AutoCameraTestActivity.REQ
...
//        subTests.put(Test.cameraAutoFocusKey, SubTest("Auto Focus"))
    ...
    private fun startMethod2Activity() {
        if (Nammu.checkPermission(Manifest.permission.CAMERA)) {
            startIntent(context, AutoCameraTestActivity::class.java, AutoCameraTestActivity.REQ)
    ...
    override fun startIntent(context: Context, target: Class<out Activity>, req: Int): Intent {
        ...
        intent.putExtra(AutoCameraTestActivity.CAMERA_TYPE_KEY, CameraFace.FACE_BACK.value)
        try {
